...
def soft_threshold_2d(feat, thres):
    abs_feat = ch.abs(feat)
    sub = abs_feat - thres
    zeros = sub - sub
    n_sub = ch.max(sub, zeros)
    one_sub = ch.ones_like(n_sub)
    n_sub = ch.where(n_sub != 0, one_sub, n_sub)
    feat = feat * n_sub
    return feat
def soft_threshold_with_ratio(beta, sparsity_ratio):
    assert len(beta.shape) == 2, beta.shape
    value_feat, _ = ch.sort(ch.abs(beta.detach()), dim=-1)
    thres_idx = int(beta.shape[1] * sparsity_ratio)
    thres = value_feat[:, thres_idx].unsqueeze(-1)
    return soft_threshold_2d(beta, thres)
...
# Elastic net loss given a loader instead with sparsity ratio
def elastic_loss_and_acc_loader_sparsity_ratio(linear, loader, sparsity_ratio, alpha, preprocess=None, family='multinomial'): 
    loss = 0
    acc=0
    n = 0
    device = linear.weight.device
    ...
    for batch in loader: 
        ...
        if preprocess is not None: 
            X = preprocess(X)
        X = soft_threshold_with_ratio(X, sparsity_ratio)
        #print(ch.sum(X[0]==0))
        bs = X.size(0)
        l,a = elastic_loss_and_acc(linear, X, y, 0, alpha, family=family)
        loss += l*bs
        acc += a*bs
        n += bs
    return loss/n, acc/n
...
def train_saga(linear, loader, lr, nepochs, lam, alpha, group=True, verbose=None, 
                ...
                preprocess=None, lookbehind=None, family='multinomial', logger=None, sparsity_ratio=False, sparsity_ratio_value=0): 
    if logger is None: 
        logger = print
    with ch.no_grad(): 
        weight, bias = list(linear.parameters())
        ...
        for t in tqdm(range(nepochs)): 
            ...
            for batch in loader: 
                ...
                if sparsity_ratio:
                    X = soft_threshold_with_ratio(X, sparsity_ratio_value)
                    #print(X.shape)
                    #print(ch.sum(X[0]==0))
                out = linear(X)
                ...
                if sparsity_ratio:
                    if alpha == 1: 
                        ...
                        if group: 
                            weight_new = group_threshold(weight_new, lr * 0 * alpha)
                        else: 
                            weight_new = soft_threshold(weight_new, lr * 0 * alpha)
                    else: 
                        # Elastic net regularization
                        if group: 
                            weight_new = group_threshold_with_shrinkage(weight_new, lr * 0 * alpha, lr * 0 * (1-alpha))
                        else: 
                            weight_new = soft_threshold_with_shrinkage(weight_new, lr * 0 * alpha, lr * 0 * (1-alpha))
                ...
                bias.data = bias_new
            if sparsity_ratio:
                saga_obj = total_loss/n_ex + 0 * alpha * weight.norm(p=1) + 0.5 * 0 * (1 - alpha) * (weight**2).sum()
            else:
...
def glm_saga(linear, loader, max_lr, nepochs, alpha, 
             ...
             family='multinomial', encoder=None, sparsity_ratio=False, sparsity_ratio_value=0): 
    ...
    for i,(lam,lr) in enumerate(zip(lams,lrs)): 
        ...
        state = train_saga(linear, loader, lr, nepochs, lam, alpha, 
                    ...
                    family=family, logger=logger, sparsity_ratio=sparsity_ratio, sparsity_ratio_value=sparsity_ratio_value)
        
        with ch.no_grad(): 
            if sparsity_ratio:
                loss,acc = elastic_loss_and_acc_loader_sparsity_ratio(linear, loader, sparsity_ratio_value, alpha, preprocess=preprocess, family=family)
            else:
            ...
            if val_loader: 
                if sparsity_ratio:
                    loss_val,acc_val = elastic_loss_and_acc_loader_sparsity_ratio(linear, val_loader, sparsity_ratio_value, alpha, preprocess=preprocess, family=family)
                else:
            ...
            if test_loader: 
                if sparsity_ratio:
                    loss_test,acc_test = elastic_loss_and_acc_loader_sparsity_ratio(linear, test_loader, sparsity_ratio_value, alpha, preprocess=preprocess, family=family)
                else:
