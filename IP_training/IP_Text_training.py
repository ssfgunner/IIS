...
parser = argparse.ArgumentParser(description='Settings for creating IP')
...
parser.add_argument("--sparsity_ratio", action='store_true', default=False, help="whether to use sparsity_ratio")
parser.add_argument("--sparsity_ratio_value", default=0.0, type=float, help="value sparsity_ratio")

def train_ip_and_save(args):
    ...
    output_proj = glm_saga(linear, indexed_train_loader, STEP_SIZE, args.n_iters, ALPHA, epsilon=1, k=1,
                      val_loader=val_loader, do_zero=False, metadata=metadata, n_ex=len(target_features), n_classes = len(classes), sparsity_ratio=args.sparsity_ratio,
                      sparsity_ratio_value=args.sparsity_ratio_value)
    W_g = output_proj['path'][0]['weight']
    b_g = output_proj['path'][0]['bias']
    
    save_name = "{}/{}_ip_{}".format(args.save_dir, args.dataset, datetime.datetime.now().strftime("%Y_%m_%d_%H_%M"))
    os.mkdir(save_name)
