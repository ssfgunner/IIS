...
from utils.utils import load_yaml, save_pkl
...
def evaluate(model, data_loader, device, target_layer='fc'):
    ...
    with torch.inference_mode():
        for i, (image, target) in enumerate(data_loader):
            ...
            with OutputHook(model, outputs=[target_layer], as_tensor=True) as h:
                output = model(image)
                returned_features = h.layer_outputs
                returned_features = returned_features[target_layer].squeeze().detach().cpu()
